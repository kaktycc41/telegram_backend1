...
# ========================
API_ID = 27877995  # <-- sizning API_ID
API_HASH = "f69a4c454706d10fea9f0e99cc91b353"  # <-- sizning API_HASH
SESSION_NAME = "session_1"  # sessiya fayl nomi
...
# ========================
async def auto_change_nick(nicknames, interval):
    """Telethon orqali nikni avtomatik o‘zgartiruvchi funksiya"""
    async with client:
        i = 0
        while True:
            current_nick = nicknames[i % len(nicknames)]
            await client(UpdateProfileRequest(first_name=current_nick))
            print(f"✅ Nik o‘zgartirildi → {current_nick}")
            i += 1
...
# ========================
async def auto_comment(channel, messages, interval):
    """Telethon orqali kanal kommentariyasiga avtomatik yozuvchi funksiya"""
    async with client:
        i = 0
        while True:
            msg = messages[i % len(messages)]
            await client.send_message(channel, msg)
            print(f"💬 {channel} kanaliga yuborildi → {msg}")
            i += 1
            await asyncio.sleep(interval * 60)
...
async def main():
    ...
    await client.start()  # Telethon sessiyani ishga tushiradi
